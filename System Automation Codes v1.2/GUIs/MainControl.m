...
and focus with the sample stage motors. It also allows the user to control
the variables that shape the grid, allowing for fine control of the grid
shape. These settings persist between sessions, and are recorded in a log
file before every run, for easy troubleshooting. The centering function
is currently only confirmed to work with the 100x magnification.

PROJECT VERSION HISTORY
08/26/17: Version 1.0
08/28/17: Version 1.1
...
09/15/17: Version 1.2
    ...
    -Added compatibility with new circular grid settings
        -NOTE: centering only works with 100x magnification, so results
...
TO DO:
    ...
    -Create launcher (possibly executable?)
        -Shortcut on desktop that launches the main program
        -Created by launcher
...
%% Warning dialog box for overwriting an existing run
overwrite = 'Ok';
fileList = dir([illum.mainPath '\Data\' dateStr]); % Gets a list of files in current folder

for i = 1:size(fileList); % Checks each file to see if filename is the same as current run name
...
if strcmp(overwrite, 'Ok')
    ...
    %% Sets all buttons and textboxes to be editable again (used if anything is set to be disabled during capture)
...
if isnan(inputDouble) %Checks if not a number, informs user that input is invalid
    ...
    while round(pos,4) ~= round(inputDouble,4)
        ...
        set(handles.text6, 'String', posmm) %Updates location indicator
        pause(.1)
        if toc > 5 % If move is taking too long
            disp('Timed out')
            break
        end
    end
...
%% 
% --- Centering button, sets default center position
...
% Re-enable all buttons and textboxes, in case they were disabled
set(handles.pushbutton1, 'Enable', 'On'); %Enables capture button
...
% Runs sample stage control program
SampleStageControl
end
...
% Closing function (runs when window is closed)
function uipanel3_DeleteFcn(varargin)

global illum
global datFol

% Saves illum structure and datFol variables to the config file
